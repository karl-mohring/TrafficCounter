...
// Constructors ////////////////////////////////////////////////////////////////
AP_OpticalFlow_ADNS3080::AP_OpticalFlow_ADNS3080(int cs_pin, int reset_pin){
	_cs_pin = cs_pin;
...
void AP_OpticalFlow_ADNS3080::set_frame_period(unsigned int period){
	byte upper = highByte(period);
	byte lower = lowByte(period);
...
unsigned int AP_OpticalFlow_ADNS3080::get_shutter_speed(){
	byte upper = read_register(ADNS3080_SHUTTER_UPPER);
	delayMicroseconds(50);  // small delay
	byte lower = read_register(ADNS3080_SHUTTER_LOWER);
	return word(upper, lower);
}
